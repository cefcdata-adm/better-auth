...
// Email-based pre-authorization: client apps register users before they first sign in
export const pendingAppAccess = pgTable(
  "pending_app_access",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    index("pending_app_access_email_client_idx").on(table.email, table.clientId),
  ],
);

// Required by better-auth jwt plugin (stores rotating RS256 key pairs)
